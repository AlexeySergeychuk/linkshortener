...
func (s *MockRepository) GetFullLink(shortLink string) string {
	args := s.Called(shortLink)
	return args.String(0)
}

func (s *MockRepository) CheckAlreadyHaveShortLink(link string) (bool, string) {
	args := s.Called(link)
	return args.Bool(0), args.String(1)
}

func (s *MockShortLinker) RandomString(link string) string {
	args := s.Called(link)
	return args.String(0)
}
...
func TestCreateLinkHandler(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			mockRepository.On("CheckAlreadyHaveShortLink", test.requestBody).Return(test.isAlreadyHaveLink, test.shortLink)

			if !test.isAlreadyHaveLink {
				mockRepository.On("SaveLinks", mock.Anything, test.requestBody).Return(test.want.responseText)
				mockShortLinker.On("RandomString", test.requestBody).Return(test.shortLink)
			}
			...
			if test.requestBody == "" {
				mockRepository.AssertNotCalled(t, "CheckAlreadyHaveShortLink", mock.Anything)
				mockRepository.AssertNotCalled(t, "SaveLinks", mock.Anything, mock.Anything)
				mockShortLinker.AssertNotCalled(t, "RandomString", mock.Anything)
			} else {
...
func TestHandler_GetLinkHandler(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			mockRepository := new(MockRepository)
			mockShortLinker := new(MockShortLinker)
			mockRepository.On("GetFullLink", mock.Anything ).Return(test.want.headerValue)
